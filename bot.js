...
const ALLOW_PORTSCAN = process.env.ALLOW_PORTSCAN === 'true';      // kunci fitur port scan
const WHATCMS_KEY = process.env.WHATCMS_KEY || '';                 // opsional
const NUMVERIFY_KEY = process.env.NUMVERIFY_KEY || '';             // opsional (numverify/apilayer)
const SPEEDTEST_BIN = process.env.SPEEDTEST_BIN || '';             // opsional: path speedtest-cli/speedtest
...
// Sanitizer sederhana (hindari injection)
...
bot.start((ctx) =>
  ctx.reply(
    [
      ...
      '/speedtest',
      '/mailcheck <email>',
      '/phoneinfo <nomor>  (butuh NUMVERIFY_KEY)',
      '/checkcms <url>     (opsional WHATCMS_KEY)',
      '/subdomains <domain>',
      '/portscan <host> <port|start-end>  (set ALLOW_PORTSCAN=true)',
    ].join('\n')
  )
);

// ==== Website Info (ringkas, no API key) ====
...
// ==== Fallback untuk perintah tak dikenal ====
bot.on('text', (ctx) => {
  if (ctx.message.text.startsWith('/')) return; // biar nggak spam
